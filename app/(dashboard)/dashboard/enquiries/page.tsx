...
function Page() {
  const [data, setData] = useState<Partial<Enquiry>[]>([]);
  ...
  const columns = useMemo<ColumnDef<Partial<Enquiry>>[]>(
    () => [
      {
        accessorKey: "email",
      ...
      {
        accessorKey: "content",
        header: () => <div className="text-left">內容</div>,
        footer: (props) => props.column.id,
      },
      {
        accessorKey: "done",
      ...
      },
      {
        ...
        header: () => <div className="text-left">改變完成狀態</div>,
        footer: (props) => props.column.id,
      },
    ],
    []
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(`/api/enquiries?${queryParams}`);
